...
from magicdrive.runner.unipad_multiview_runner import UVTRSSL_Wrapper
from magicdrive.pipeline.infer_uvtrssl import InferUVTRSSL
from magicdrive.pipeline.pipeline_controlnet_1x1 import StableDiffusionControlNetPipeline1x1
...
def build_pipe(cfg, device):
    ...
    # UVTRSSL_Wrapper_cls = load_module(cfg.model.UVTRSSL_Wrapper_module)
    # UVTRSSL_Wrapper_path = os.path.join(cfg.resume_from_checkpoint, cfg.model.UVTRSSL_Wrapper_dir)
    # logging.info(f"Loading UVTRSSL_Wrapper from {UVTRSSL_Wrapper_path} with {UVTRSSL_Wrapper_cls}")
    # UVTRSSL_Wrapper = UVTRSSL_Wrapper_cls.from_pretrained(
    #     UVTRSSL_Wrapper_path, torch_dtype=weight_dtype)
    # UVTRSSL_Wrapper.eval()
    # # pipe_param["UVTRSSL_Wrapper"] = UVTRSSL_Wrapper
    # UVTRSSL_Wrapper = UVTRSSL_Wrapper.to(device)
    
    # InferUVTRSSL_cls = InferUVTRSSL(UVTRSSL_Wrapper)
    ...
    # return pipe, weight_dtype, InferUVTRSSL_cls
...
def prepare_all(cfg, device='cuda', need_loader=True):
    ...
    # pipe, weight_dtype, InferUVTRSSL_cls = build_pipe(cfg, device)
    ...
    # return pipe, val_dataloader, weight_dtype, InferUVTRSSL_cls
...
def run_one_batch_pipe(
    ...
    pipe: StableDiffusionControlNetPipeline1x1,
...
# def run_one_batch(cfg, pipe, val_input, weight_dtype, InferUVTRSSL_cls, global_generator=None,
#                   run_one_batch_pipe_func=run_one_batch_pipe,
#                   transparent_bg=False, map_size=400):
    ...
    # val_input['unipad']['img'] = torch.tensor(val_input['unipad']['img'], dtype=weight_dtype, device=pipe.device)
    # val_input['unipad']['img'] = torch.tensor(val_input['unipad']['img'], device=pipe.device)
    # val_input['camera_param'] = torch.tensor(val_input['camera_param'], device=pipe.device)
    # controlnet_image = InferUVTRSSL_cls(val_input)

    controlnet_image = torch.tensor(val_input['syntheocc']['ctrl_img'], dtype=weight_dtype, device=pipe.device)
