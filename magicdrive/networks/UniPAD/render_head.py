...
from magicdrive.networks.UniPAD.render_utils import models
from magicdrive.networks.UniPAD.utils import sparse_utils
from magicdrive.networks.UniPAD import utils
...
# class RenderHead(nn.Module):
    ...
    def forward(self, pts_feats, img_feats, rays, img_metas, img_depth):
        ...
        # cur_sample_idx = img_metas[0]['sample_idx']

        # cur_train_extracted_bev_feature = uni_feats.cpu().to(dtype=torch.float16)
        # torch.save(cur_train_extracted_bev_feature, f'./train_extracted_bev_feature/{cur_sample_idx}.bin')

        # cur_val_extracted_bev_feature = uni_feats.cpu().to(dtype=torch.float16)
        # torch.save(cur_val_extracted_bev_feature, f'./val_extracted_bev_feature/{cur_sample_idx}.bin')
